  ...
  const fetchOrderHistory = async () => {
    try {
      ...
      const response = await fetch(
        `https://freshgrupo-server.onrender.com/api/orders/${currentUser.id}`,
      );
      if (response.ok) {
  ...
  const fetchPackDetails = async order => {
    try {
      ...
      if (order.isCustom && order.customPackItems) {
        let itemsArray = order.customPackItems;
        if (typeof order.customPackItems === 'string') {
          try {
            itemsArray = JSON.parse(order.customPackItems);
          } catch (err) {
            itemsArray = [];
          }
        }
      ...
      // Normal Pack with packProducts from backend
      if (order.packProducts && order.packProducts.length > 0) {
        setPackDetails({
          name: order.Pack?.name || `Pack #${order.packId}`,
      ...
      if (order.id) {
        const response = await fetch(
          `https://freshgrupo-server.onrender.com/api/orders/details/${order.id}`,
        );
        if (response.ok) {
          const orderDetails = await response.json();
          if (
            orderDetails.packContents &&
            orderDetails.packContents.length > 0
          ) {
            setPackDetails({
              name: orderDetails.Pack?.name || `Pack #${order.packId}`,
            ...
            <View style={styles.row}>
              ...
              <TouchableOpacity onPress={() => fetchPackDetails(item)}>
                <Text style={[styles.value, styles.clickableText]}>
                  {item.isCustom
                    ? item.customPackName || 'Custom Pack'
                    : item.Pack?.name || `Pack #${item.packId}`}
                </Text>
              </TouchableOpacity>
            </View>
          )}
