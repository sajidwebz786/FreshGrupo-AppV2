...
const CartScreen = () => {
    ...
    try {
      ...
      if (currentUser && currentUser.id) {
        const response = await fetch(
          `https://freshgrupo-server.onrender.com/api/addresses?userId=${currentUser.id}`,
        );
        ...
        prevItems.map(item =>
          item.id === cartItemId
            ? {
                ...
                totalPrice: (newQuantity * item.unitPrice).toFixed(2),
              }
            : item,
        ),
      );
  ...
  const addNewAddress = async () => {
    ...
    try {
      ...
      const response = await fetch(
        'https://freshgrupo-server.onrender.com/api/addresses',
        {
          method: 'POST',
          ...
          <View>
                ...
                <View style={styles.cartTopRow}>
                  <View>
                    <Text style={styles.itemName}>
                      {item.isCustom
                        ? item.customPackName
                        : item.Pack?.name || 'Unknown Pack'}
                    </Text>
                    <Text style={styles.itemType}>
                      {item.isCustom
                        ? '🛒 Custom Pack'
                        : `⏰ ${item.Pack?.PackType?.name || 'Unknown Type'}`}
                    </Text>
                  </View>
            ...
            <View style={styles.addressSection}>
              ...
              <Text style={styles.instructionText}>
                Please select from any of the below addresses for delivery or
                create new address.
              </Text>

              {addresses.length === 0 && !showAddNew ? (
                <Text style={styles.noAddressText}>
                  No addresses saved. Add your first delivery location.
                </Text>
              ) : (
                addresses.map(addr => (
                  <TouchableOpacity
                ...
                <View style={styles.addNewContainer}>
                  ...
                  <View style={styles.typeSelector}>
                    <TouchableOpacity
                      style={[
                        styles.typeButton,
                        newAddressType === 'home' && styles.typeSelected,
                      ]}
                      onPress={() => setNewAddressType('home')}>
                      <Text
                        style={[
                          styles.typeText,
                          newAddressType === 'home' && styles.selectedTypeText,
                        ]}>
                        Home
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[
                        styles.typeButton,
                        newAddressType === 'work' && styles.typeSelected,
                      ]}
                      onPress={() => setNewAddressType('work')}>
                      <Text
                        style={[
                          styles.typeText,
                          newAddressType === 'work' && styles.selectedTypeText,
                        ]}>
                        Work
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[
                        styles.typeButton,
                        newAddressType === 'other' && styles.typeSelected,
                      ]}
                      onPress={() => setNewAddressType('other')}>
                      <Text
                        style={[
                          styles.typeText,
                          newAddressType === 'other' && styles.selectedTypeText,
                        ]}>
                        Other
                      </Text>
                    </TouchableOpacity>
            ...
            <View style={styles.summaryCard}>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={[styles.summaryValue, {color: '#1a8b43'}]}>
                  Free
                </Text>
              </View>
