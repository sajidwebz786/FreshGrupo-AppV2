...
const CustomPackScreen = () => {
  ...
  const handleAddToCart = async () => {
    ...
    const selectedItems = Object.entries(selectedProducts).filter(
      ([_, qty]) => qty > 0,
    );
    ...
    if (selectedItems.length === 0) {
      Alert.alert(
        ...
        'Please select at least one product for your custom pack.',
      );
      return;
    }
    ...
    try {
      ...
      const customPackData = {
        ...
        customPackName: `${category} Custom Pack`,
        customPackItems: JSON.stringify(customPackItems),
        unitPrice: totalValue, // ✅ ADD THIS LINE
      };
      ...
      if (response) {
        Alert.alert('Success', 'Custom pack added to cart successfully!', [
          {text: 'Continue Shopping', style: 'default'},
          {
            text: 'Go to Cart',
            onPress: () => navigation.navigate('Cart'),
  ...
  const getProductIcon = name => {
    ...
    if (n.includes('chicken')) return '🍗';
    if (n.includes('fish')) return '🐟';
    if (n.includes('meat') || n.includes('beef') || n.includes('pork'))
      return '🥩';
    // Grains and Staples
    if (n.includes('rice')) return '🍚';
    if (n.includes('wheat') || n.includes('flour')) return '🌾';
    ...
    if (
      ...
      n.includes('multi vitamin') ||
      n.includes('vitamin')
    )
      return '🧴';
    if (n.includes('omega') || n.includes('capsule') || n.includes('caps'))
      return '💊';
    if (n.includes('calcium') || n.includes('tabs')) return '🦴';
    if (n.includes('supplement')) return '🧪';
    ...
    if (n.includes('honey')) return '🍯';
    if (n.includes('jam')) return '🍯';
    if (n.includes('nut') || n.includes('almond') || n.includes('cashew'))
      return '🥜';
    ...
    <View style={styles.container}>
      ...
      <View style={styles.categorySelector}>
        <Text style={styles.selectorTitle}>
          Select Products for {category} Custom Pack
        </Text>
        <Text style={styles.minValueText}>
          Minimum value: ₹{weeklyPackValue}
        </Text>
        {/* <Text style={styles.crossCategoryCaption}>
          💡 You can add items from any category to meet the minimum value requirement
        </Text>
      ...
      <ScrollView
        ...
        <Text style={styles.sectionTitle}>Available {category} Products</Text>
        <View style={styles.productsContainer}>
          {getCurrentCategoryProducts().map(product => (
            <View key={product.id} style={styles.productCard}>
              <View style={styles.productIconContainer}>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[
            styles.addToCartButton,
            (addingToCart || totalValue < weeklyPackValue) &&
