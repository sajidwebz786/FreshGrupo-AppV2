...
class ApiService {
  ...
  async verifyCreditPayment(razorpayPaymentId, razorpayOrderId, razorpaySignature, transactionId, token) {
    return this.doFetch('/wallet/purchase/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ razorpayPaymentId, razorpayOrderId, razorpaySignature, transactionId }),
    });
  }
